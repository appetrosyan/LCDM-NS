...
\begin{enumerate}
\item one multiplication in the argument of the prior\footnote\{That is actually not true in Python, as it is unable to rewrite and optimise functions in general. The reason being that dynamic typing allows people to override the operator \texttt{\_\_pow\_\_}, and hence the identity \((\exp [a])^{b} = \exp [ab]\) may no longer hold for custom types. However, Cythonized versions of the program, may take advantage of the opitimisation.\}
