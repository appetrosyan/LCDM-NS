...
\wh{``distributed'' is quite a specialised word here, out of context with the more general discussion. Something like ``Which parameters of the model are preferred by the data'' would be better}
...
\wh{I'm not sure I agree that this is ``important'' for cosmology specifically. For sophisticated instrumentation you need to introduce nuisance parameters and marginalise them out. It is a property of current cosmological analyses that these nuisance parameters are ``faster'', and it is computationally very convenient to exploit this, but this is not a necessity}
...
and very little prior knowledge. Hence a large number of algorithms were developed to speed the computation up: Metropolis-Hastings \citep{Metropolis} used in conjunction with Gibbs sampler \citep{Metropolis-hastings-gibbs} and more recently Nested Sampling \citep{skilling2006}, which will be our focus. \wh{Also mention Hamiltonian Monte Carlo}

Nested Sampling, as described by \citeauthor{skilling2006} is rather abstract, and multiple algorithmically distinct implementations of the idea exist \citep{Feroz2009MultiNestAE}, polychord\wh{Fix citation \& list. Also mention \texttt{MultiNest} and \texttt{dynesty}}. However, \cite{chen-ferroz-hobson} noted that the algorithm is sensitive to how two quantities lieklihood and prior are defined, with respect to what the posterior distribution, and hence called the technique\wh{Sentence doesn't quite make sense -- which technique} \emph{automatic power posterior repartitioning} (PPR). While \citeauthor{chen-ferroz-hobson} used PPR to improve the stability of convergence for prior distributions that may have been at variance with the true posterior, the idea of repartitioning can be used also to speed up the convergence (as if our prior knowledge had been more precise), without biasing the sampler and altering the results: posterior distribution and evidence. \wh{Make it clear that this is one of the focusses of this project, rather than a property explored in the literature}
...
becomes \wh{Maybe consider \(\mathcal{Z}\) rather than \(Z\)}
\begin{equation}
...
that the model accurately describes the physical process in question. \wh{Consider linking it to $P(D|M)$ and $P(M|D)$}
...
distinct iso-likelihood hyper-surface\footnote{think isotherms  where likelihood correponds to temperature.\wh{I would avoid this analogy, since ``temperature'' has its own meaning in inference}}. Each hyper surface
...
\begin{eqnarray}
  ...
  \vdots\\
\end{eqnarray}
\wh{I like this way of expressing prior volume contraction. However, you should definitely use $X$ rather than $Z$ to denote prior volume. You should also comment that this relationship is not exact, but only statistically true on average, but we can quantify the errors and propagate them accurately to a final error on the evidence}
...
efficient\footnote{thankfully, Polychord uses a technique called slice   sampling that tries to interpolate\wh{Interpolate isn't quite right here. ``Navigate the likelihood-constrained region efficiently'' would be better} the likelihood re-using as much  information obtained from the current iteration as possible.}.
...
\end{equation}
Now we must\footnote{A thorough discussion of why this is necessary is given in the Appendix \autoref{sec:repart-necessity}}\wh{From \autoref{eqn:PPR}, don't we formally also need a factor of $\pi^{1-\beta}$?} change the likelihood function to 
\begin{equation}
...
domain of our prior. This fact will be important later.\label{domain-discussion}\wh{I like this -- makes things very clear}
...
\wh{I think it would be good to try to emphasise the overall goal here: Nested sampling is expensive because of the original prior compression phase. In general it's quite cheap to locate the peak, and rough size of a posterior (think MH/HMC), which ideally could then be used to speed up evidence calculation}
...
\end{equation}
\wh{This notation is not very clear for the uniform component. Here it might be useful to introduce the notation the boolean function $[X]$, so a uniform prior density is $\frac{[a<\theta<b]}{b-a}$. You can see this in more detail in this text: \url{http://www.mrao.cam.ac.uk/~wh260/Galileo/Books/Maths/ConcreteMathematics.pdf}}
...
\wh{One could of course solve numerically, which wouldn't be that expensive (particularly relative to the cosmological likelihood). Since the functions are all monotonic, bisecting would be a very stable approach.}
...
preferred direction: if our original prior was \wh{were --- I'm a stickler for the subjunctive} uniform, the
...
case occur.
\wh{Does this actually follow? Could the same argument not be applied to any other repartitioning scheme?}
...
\wh{I appreciate that the domain issue makes this impractical, but if I were setting up a box prior, I would phrase it as
    \begin{align}
        \pi(\theta,\beta) &= \frac{[a(\beta) < \theta < b(\beta)]}{b(\beta) - a(\beta)}\\
        a(\beta) &= \ldots \\
        b(\beta) &= \ldots 
    \end{align}
    where $a(\beta)$ and $b(\beta)$ run smoothly from the edges of the hypercube, through $\theta_0\pm \sigma$, to $\theta_0$, where $\theta_0$ is a location and $\sigma$ a width.
}
...
\wh{Let's discuss this in meeting}
...
prior branch.
\wh{It would be good to have a bit more detail on how mixture repartitioning is implemented}

\begin{figure}
 \input{./illustrations/mixture.tex}
\caption{\label{orga504aba}
    An example of a mixture repartitioning. Notice that the mixture is not normalised to emphasise the coincidence of values with both the uniform distribution and a Gaussian.\wh{Both of your figures are excellent. More of these please --- they can be a great way to reduce the number of words.}}
\end{figure}
...
This figure illustrates characteristics of different repartitioning schemes. The X axis shows the evidence estimates for the last 1000 iterations of the algorithm \wh{Is this what is being plotted?}. The correct value for evidence is obtained from the Uniform prior, and is labelled reference. PPR converged faster, hence most late likelihood calls were from the typical set, and it's a much narrower peak at the correct value. mixture repartiotioning converged even faster than that, having drawn more points form the typical set. The deliberately incorrect repartitioning scheme with the resizeable box model is also given.}
\end{figure}
...
standard deviation}.\wh{Possibly comment on the normalisation -- it's not necessary, but helpful to make the evidence integrate to give 1/prior volume}
