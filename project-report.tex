...
\begin{table}
  ...
  \begin{tabular}{llrrr}
    \textbf{Prior} & \textbf{Device used} & ${\cal D}\{ {\cal P}, \bar{\cal P}\}$ & ${\cal Z}$ & $n_\text{live}$\\
    \hline
...
\begin{table}
  \centering
  \caption{Performance metrics for Cosmology runs using
    Cobaya. Predicted values are crude linear extrapolations of the
    respective series. Precision normalisation assumes errors in
    ${\cal Z}$ scale as $n_\text{live}^{-1}$. }
  \begin{tabular}{llrrr}
    \textbf{Prior} & \textbf{Device used} & \textbf{Time to completion/hrs.} & \({\cal N}\{ {\cal L}\}\) & $n_\text{live}$\\
    \hline
    Uniform & CSD3 &\( 32.2 \) & \(480 000\) & 108\\
    SSIM\((U, G)\) & CSD3 & \(1.7\) & \(90 000\) & 100\\
    SSIM\((U, G)\) & PC & \(50\) & \(49 000\) & 50\\
    \hline
    Uniform & PC(Projected) & \(912\) & \(240 000\) & 50\\
    Uniform (precision-normalised) & CSD3 &  \(224\) & 3 360 000  & 700
  \end{tabular}
...
\begin{figure}
  ...
  \caption{The marginalised posteriors for \texttt{Cobaya} +
    ...
    more tightly constrained. } \label{fig:cosmology}
\end{figure}
\end{landscape}
