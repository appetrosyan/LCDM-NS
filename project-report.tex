...
\item \textbf{Representation}. In prior hyperspace
  ...
  represented in \( \hat{\pi}
  (\bm{\hat{\theta}})\). \label[Property]{spec-prop}
  
\item \textbf{Convergence}. The sampling favours configurations $\bm\hat{\theta} \in \Psi_{R}$. \label[Property]{vconv-prop}
\item \textbf{Objectivity}. The bias towards $\bm\hat{\theta}$ regions of large \( \hat{\cal P}(\bm{\hat{\theta}})\) is stronger than the bias towards \( \hat{\pi} (\bm{\hat{\theta}})\). \label[Property]{obj-prop}
\end{enumerate}
