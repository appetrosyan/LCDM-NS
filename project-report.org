...
** Brief primer on Bayesian inference. 
   ...
   \begin{equation} 
    {\cal Z} = \int_{\Psi} {\cal L}(\theta) \pi(\theta) d\theta, \label{eq:def-z}
   \end{equation}
...
** Nested Sampling.
   ...
   \begin{equation}
	 \begin{array}
	 X_{0} = &1,\\
	 X_{1} = &X_{0} - \frac{X_{0}}{n_\text{live}},\\
	 \vdots\\
	 X_{m} = &X_{m-1} - \frac{X_{m-1}}{m_\text{live}},\\
	 \vdots\\
   \end{array}
   ...
   autoref:eq:recurrence-relation is not exact. However, $P(\Delta)$
   ...
   \begin{equation}
    \begin{align}
     {\cal P}(\theta) = \pi(\theta) & \forall \theta.
    \end{align}
   \end{equation}
   ...
   at $g=9.81$, with standard deviation $\sigma_{g} = 0.01$. 
   \[{\cal P}(\theta) = N(9.81, 0.01)\]. 
...
*** Additive mixtures.
	...
	\begin{equation}label:eq:additive-mix
	  \pi(\theta) = \frac{1}{Z} \left\lbrace \beta (b - a)^{-1} + (1-\beta) \exp \left[ -\left(\frac{\theta - \mu}{\sigma} \right)^{2}\right]\right\rbrace \cdot \left[ \mathbf{1}(\theta-a) - \mathbf{1}(\theta-b)\right].
	\end{equation}
	where $\mathbf{1}$ is the Heaviside step function. Integrate to
