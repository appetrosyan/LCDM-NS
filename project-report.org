...
** Brief primer on Bayesian inference. 
   ...
   \begin{equation}
    \pi : \text{unit hyper-cube} \rightarrow \Psi,
   \end{equation}
...
** Power posterior re-partitioning
   ...
   uni-variate examples, where the relative impact is maximal, it's not
...
*** Additive mixtures.
	...
	for analytically, as one would with PPR[fn::one could argue
